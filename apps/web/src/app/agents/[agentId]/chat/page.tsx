      ...
      <div className="flex items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={async () => {
              setError(null);
              const res = await apiFetch(`/v1/agents/${agentId}/clear_context`, { method: "POST", body: JSON.stringify({}) });
              if (!res.ok) setError(`Failed to clear context (${res.status}): ${await res.text().catch(() => "")}`);
              await loadAgent();
              await loadActive();
            }}
          >
          ...
          <Button
            ...
            onClick={async () => {
              setError(null);
              const res = await apiFetch(`/v1/agents/${agentId}/sleep`, { method: "POST", body: JSON.stringify({ clearContext: true }) });
              if (!res.ok) setError(`Failed to sleep (${res.status}): ${await res.text().catch(() => "")}`);
              await loadAgent();
            }}
          >
            Sleep
          ...
          <Button
            ...
            onClick={async () => {
              setError(null);
              const res = await apiFetch(`/v1/agents/${agentId}/wake`, { method: "POST", body: JSON.stringify({}) });
              if (!res.ok) setError(`Failed to wake (${res.status}): ${await res.text().catch(() => "")}`);
              await loadAgent();
            }}
          >
            Wake
