...
export default function AgentsPage() {
  ...
  const [editTools, setEditTools] = useState<{ agentId: string; toolIds: string[]; lockedToolIds: string[] } | null>(null);
  ...
  const load = async () => {
    setError(null);
    const [aRes, pRes, sRes, gRes, tRes] = await Promise.all([
    ...
    if (!tRes.ok) {
      setError(`Failed to load tools (${tRes.status})`);
      return;
    }
  ...
  const groupToolIds = useMemo(() => {
    const names = new Set(["group_get_info", "group_get_members", "group_get_messages"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const skillToolIds = useMemo(() => {
    const names = new Set(["read_file_lines", "linux_command", "skill_rate", "skill_get_ratings"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const sessionToolIds = useMemo(() => {
    const names = new Set(["agent_list_sessions", "agent_get_session_messages", "agent_search_messages"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const supervisorToolIds = useMemo(() => {
    const names = new Set(["agent_dispatch_run", "email_send", "agent_send_mail", "agent_wake_agent"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const guardianToolIds = useMemo(() => {
    const names = new Set(["system_logs_recent", "propose_patch"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const supervisorProjectToolIds = useMemo(() => {
    const names = new Set(["project_create", "project_assign_lead"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);

  const projectLeadToolIds = useMemo(() => {
    const names = new Set(["group_create", "group_set_owner"]);
    return tools.filter((t) => names.has(t.name)).map((t) => t.id);
  }, [tools]);
  ...
  return (
    <main className="grid gap-6">
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        onClick={() =>
                          setEditTools(() => {
                            const inAnyGroup = (a.groupMembers ?? []).length > 0;
                            const locked = inAnyGroup ? groupToolIds : [];
                            const hasSkills = (a.skillPaths ?? []).length > 0;
                            const locked2 = hasSkills ? skillToolIds : [];
                            const hasSessions = ((a as any)._count?.sessions ?? 0) > 0;
                            const locked3 = hasSessions ? sessionToolIds : [];
                            const locked4 = a.isSupervisor ? supervisorToolIds : [];
                            const locked5 = (a as any).isGuardian ? guardianToolIds : [];
                            const locked6 = a.isSupervisor ? supervisorProjectToolIds : [];
                            // Heuristic: if agent is lead of any project, backend will enforce. In UI we keep them unlocked by default.
                            const base = (a.agentTools ?? []).map((x) => x.tool.id);
                            const allLocked = Array.from(new Set([...locked, ...locked2, ...locked3, ...locked4, ...locked5, ...locked6]));
                            return { agentId: a.id, toolIds: Array.from(new Set([...base, ...allLocked])), lockedToolIds: allLocked };
                          })
                        }
                      >
                        工具
      ...
      {editTools ? (
        <div className="fixed inset-0 bg-black/30 p-4 grid place-items-center" onMouseDown={() => setEditTools(null)}>
          <div className="w-full max-w-3xl" onMouseDown={(e) => e.stopPropagation()}>
            <Card>
              <CardHeader>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-lg font-semibold">Edit agent tools</div>
                    <div className="mt-1 text-sm text-slate-600">Select or unselect tools for this agent.</div>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => setEditTools(null)}>
                    Close
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="grid gap-3">
                {tools.length === 0 ? (
                  <div className="text-sm text-slate-600">No tools yet. Create one in <Link className="underline" href="/tools">Tools</Link>.</div>
                ) : (
                  <div className="grid gap-2 rounded-xl bg-slate-50 ring-1 ring-slate-200 p-3 max-h-[420px] overflow-auto">
                    {tools.map((t) => {
                      const checked = editTools.toolIds.includes(t.id);
                      const locked = editTools.lockedToolIds.includes(t.id);
                      return (
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              setEditTools((prev) => {
                                if (!prev) return prev;
                                const next = e.target.checked
                                  ? Array.from(new Set([...prev.toolIds, t.id]))
                                  : prev.toolIds.filter((x) => x !== t.id);
                                return { ...prev, toolIds: Array.from(new Set([...next, ...prev.lockedToolIds])) };
                              });
                            }}
                          />
                          <div>
                            <div className="text-sm font-medium">
                              {t.name} {locked ? <span className="text-[11px] text-slate-500">(locked)</span> : null}
                            </div>
                            <div className="text-xs text-slate-600">{t.description}</div>
                            <div className="text-[11px] text-slate-500 font-mono break-all">{t.id}</div>
                          </div>
                        </label>
                      );
                    })}
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="secondary" onClick={() => setEditTools(null)}>
                    Cancel
                  </Button>
                  <Button
                    onClick={async () => {
                      setError(null);
                      const res = await apiFetch(`/v1/agents/${editTools.agentId}/tools`, {
                        method: "PUT",
                        body: JSON.stringify({ toolIds: Array.from(new Set([...editTools.toolIds, ...editTools.lockedToolIds])) })
                      });
                      if (!res.ok) {
                        setError(`Failed to update tools (${res.status}): ${await res.text().catch(() => "")}`);
                        return;
                      }
                      setEditTools(null);
                      await load();
